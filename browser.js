...
const TensorView = require('./tensorview.js')
...
class Boltzman{
  ...
  trainp(data){
    ...
    var qresult = qprob.greaterEqual($.variable({init:'randomUniform', shape: [data.shape[0], this._shape[1]]}).layer).asType('float32')
...
//  var hft = require('../audio/fft/hft')
  ...
  let size = 784
  let epochas = Number(argv.e) || 1
  let embed = 4
  var brainz = new Array(10).fill(0).map((e, i) => new Boltzman(size, size * (argv.s || 16), argv.r || 5e-2,  .9, 1e-6, .1, i))
  train()
 
  function train(){
    let batches = new Array(10).fill(0).map(e => [])

    let batchSize = argv.b ||  1
    var ready = () => batches.map(e => e.length >= batchSize).filter(Boolean).length === batches.length
    var go = false
    while(!ready()){
      let data = mnist.nextTrainBatch(batchSize)
      let image = data.image.reshape([batchSize, size]).cast('float32')
  ...
  function validate(){
    var bm = new Boltzman(size, size * (argv.s || 16), argv.r || 1e-3,  .9, 1e-6, .1, 10)
    bm.weights = brainz.slice(1).reduce((a, e) => a.add(e.weights), brainz[0].weights)
    let biew = new TensorView(bm._shape, [1,1])
    biew.append(document.body)
    biew.draw(bm.weights)
//    return
    batches.forEach(batch =>{
  tf.tidy(_=>{
    tf.unstack(bm.activate(bm.query(batch).result).result).slice(0, 1).forEach(e => { 
      //console.vlog(e.dataSync().join(''))
      let view = new TensorView([28,28], [1,1])
...
function brainDump(){
  brainz.forEach(e => {
    console.log(e)
    let view = new TensorView(e._shape, [1,1])
    view.append(document.body)
    view.draw(e.weights)
